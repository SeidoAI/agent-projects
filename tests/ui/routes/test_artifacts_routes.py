...
def artifact_client(
    ...
    (
        artifact_project / "instances" / "sessions" / "session-a" / "artifacts" / "task-checklist.md"
    ).write_text("| # | | |\n|---|---|---|\n| 1 | task | done |\n")
...
class TestGetArtifact:
    ...
    def test_missing_file_returns_404(
        ...
        plan = artifact_project / "instances" / "sessions" / "session-a" / "artifacts" / "plan.md"
        plan.unlink()
        r = artifact_client.get(
            f"/api/projects/{artifact_project_id}/sessions/session-a/artifacts/plan"
        )
...
class TestApproveArtifact:
    def test_happy_path_writes_sidecar(
        ...
        sidecar = artifact_project / "instances" / "sessions" / "session-a" / "plan.approval.yaml"
    ...
    def test_accepts_uppercase_sequential_session_key(
        ...
        assert r.status_code == 200
        sidecar = artifact_project / "instances" / "sessions" / "TST-S1" / "plan.approval.yaml"
        assert sidecar.exists()
