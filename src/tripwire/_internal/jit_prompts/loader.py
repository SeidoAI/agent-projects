...
Loads built-in JIT prompts from ``manifest.yaml`` next to this module
and merges in any project-local extras declared in
``project.yaml.jit_prompts.extra``. Returns a dict keyed by lifecycle
event whose values are lists of instantiated JIT prompts.

The whole-project disable (``jit_prompts.enabled: false``) is enforced
...
def load_jit_prompt_registry(project_dir: Path) -> dict[str, list[JitPrompt]]:
    ...
    Returns ``{}`` when ``project.yaml.jit_prompts.enabled`` is False.
...
def _find_jit_prompt_class(module: Any, declared_id: str | None) -> type[JitPrompt]:
    """Find the single JitPrompt subclass in a project-local module."""
    candidates = [
        v
        for v in vars(module).values()
        if isinstance(v, type) and issubclass(v, JitPrompt) and v is not JitPrompt
    ]
    if declared_id:
        for c in candidates:
...
def _read_jit_prompts_block(project: ProjectConfig) -> dict:
    ...
    The :class:`ProjectConfig` model owns the public shape. This helper
    returns a plain dict so ``fire_jit_prompt_event`` and registry loading
    do not need to know the typed Pydantic surface.
    """
