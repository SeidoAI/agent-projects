...
@router.get("/prs", response_model=list[PRSummary], responses=V2_RESPONSES)
async def list_prs(repo: str, head: str | None = None) -> list[PRSummary]:
    raise_v2_not_implemented(_DETAIL)


@router.get(
    "/prs/{pr_number}/checks", response_model=list[CheckRun], responses=V2_RESPONSES
)
...
@router.get(
    "/prs/{pr_number}/reviews", response_model=list[Review], responses=V2_RESPONSES
)
