"""``tripwire session artifacts`` — alias for ``tripwire project artifacts list``."""

from __future__ import annotations

from tripwire.cli.project.artifacts import artifacts_list
from tripwire.cli.session._group import session_cmd

# Alias `tripwire session artifacts <id>` to the existing
# `tripwire project artifacts list <id>`. Uses `add_command` (not the
