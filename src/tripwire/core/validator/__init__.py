...
# Auto-fix machinery lives in :mod:`tripwire.core.fix` (extracted in
# v0.13.1 so the validator module stays mutation-free per §3). The
# orchestrator below imports ``apply_fixes`` lazily — exposing it as
# ``tripwire.core.validator.apply_fixes`` for back-compat with the few
# callers that still import from this path.
from tripwire.core.fix import apply_fixes  # noqa: E402  (after fix-section anchor)
