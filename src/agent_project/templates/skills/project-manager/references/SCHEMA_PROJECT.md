...
The root config file for an agent-project repo. Defines the project's
...
# Sequential key counters — DO NOT hand-edit. Use `agent-project next-key`.
next_issue_number: 1
...
- **`next_issue_number` / `next_session_number`** — use
  `agent-project next-key`. Hand-editing these causes sequence drift
