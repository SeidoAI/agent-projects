...
````markdown
Run `agent-project refs list SEI-42` to see the references:
...
- `agent-project refs list <issue-key>` — every reference in one issue
- `agent-project refs reverse <node-id>` — every entity that references a node
- `agent-project refs check` — full scan (dangling, orphan, stale)
