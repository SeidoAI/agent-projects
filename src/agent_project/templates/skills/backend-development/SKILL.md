...
description: >-
  Development skill for a Python backend repo in an agent-project. Covers
  branching, test-driven development, commit patterns, code quality
  checks, agent-project interaction (reading issues, writing comments,
...
agent-project. You have been given one or more issue keys and the
...
This is an agent-project repo. The PM skill manages issues, concept
...
21. **Run the target repo's checks**:
    ...
    ```
22. **Run `agent-project refs check`** to confirm no dangling or stale
