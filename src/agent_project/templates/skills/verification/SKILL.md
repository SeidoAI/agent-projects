...
license: MIT
metadata:
  author: agent-project
  version: "1.0"
compatibility: >-
  Any target code repo. Requires `gh` CLI for PR operations. Uses
  the agent-project CLI for concept graph freshness checks.
---
...
- Run `agent-project` CLI commands against the project repo
...
12. **Identify nodes the PR touched**. For every source path in the
    ...
    `agent-project refs reverse` or read the cache directly).
...
17. **If all checks pass**, post an approve review:
    ...
    - `agent-project validate --strict` → exit 0
    - `agent-project node check` → all fresh
...
18. **If any check fails**, post a request-changes review with
    ...
    After fixing, re-run `agent-project validate --strict` and
...
21. **Run `agent-project validate --strict`** on the project repo
